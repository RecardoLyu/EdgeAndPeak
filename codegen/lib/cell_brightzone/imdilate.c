              ...
              emxArray_boolean_T *B)
{
  ...
  if (pre_pad) {
    ...
    padarray(Apadpre, pad_lr_data, Apad);
    Apad_data = Apad->data;
  ...
  for (sInd = 0; sInd < i; sInd++) {
    ...
    offset = 0;
    exitg1 = false;
    while ((!exitg1) && (offset + 1 <= loop_ub_tmp)) {
      if ((allheightIn_data[offset] == 0.0) ||
          rtIsNaN(allheightIn_data[offset])) {
        offset++;
      } else {
        strel_is_all_flat = true;
        exitg1 = true;
